...
logger.history = function()
	...
	if not history[start] then
		return history
	end
	if start > histSize then
		start = 1
	else
		tmove(history, 1, start - 1, histSize + 1)
	end -- append
	...
	if lvl or flt then
		...
		for _, e in ipairs(hist) do
			if
				...
				and (
					not flt
					or sfind(case and e.id or slower(e.id), flt, 1, true)
					or sfind(case and e.message or slower(e.message), flt, 1, true)
				)
			then
				filt[#filt + 1] = e
			end
...
local lf = function(loglevel, lvl, id, fmt, ...)
	if histSize <= 0 and loglevel < lvl then
		return
	end
	local ct = time()
	local msg = sformat(fmt, ...)
	if histSize > 0 then
		store({ time = ct, level = lvl, id = id, message = msg })
	end
	...
	if ct - lasttime > 0 or lvl < 3 then
		stime = date("%X")
		lasttime = ct
	end
	if id == lastid and lvl > 3 then
		id = idempty
	else
		lastid = id
	end
...
local l = function(loglevel, lvl, id, ...)
	if histSize > 0 or loglevel >= lvl then
		return lf(loglevel, lvl, id, srep("%s", select("#", ...), " "), ...)
	end
end
...
logger.idLength = function(len)
	...
	if type(len) ~= "number" or len < 4 then
		error("len must be a number >=4", 2)
	end
	len = min(len, 40)
	idlen = len
	idf = "%" .. len .. "." .. len .. "s:"
...
function logger.new(id, loglevel)
	...
	local r = {
		...
		e = function(...)
			return l(loglevel, ERROR, id, ...)
		end,
		w = function(...)
			return l(loglevel, WARNING, id, ...)
		end,
		i = function(...)
			return l(loglevel, INFO, id, ...)
		end,
		d = function(...)
			return l(loglevel, DEBUG, id, ...)
		end,
		v = function(...)
			return l(loglevel, VERBOSE, id, ...)
		end,

		ef = function(fmt, ...)
			return lf(loglevel, ERROR, id, fmt, ...)
		end,
		wf = function(fmt, ...)
			return lf(loglevel, WARNING, id, fmt, ...)
		end,
		f = function(fmt, ...)
			return lf(loglevel, INFO, id, fmt, ...)
		end,
		df = function(fmt, ...)
			return lf(loglevel, DEBUG, id, fmt, ...)
		end,
		vf = function(fmt, ...)
			return lf(loglevel, VERBOSE, id, fmt, ...)
		end,
	}
	r.log = r.i
	r.logf = r.f
