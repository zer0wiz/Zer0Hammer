...
function obj:toggleModalTray(cscreen, trayColor, text)

    local cres = cscreen:fullFrame()
-- tray_width -[ 0 ~ 1 ] screen rate
    local tray_width = 400
    local tray_height = 60
    local tray_padding_top = 400
    local tray_padding_left = 0
    local tray_alpha = 0.8
    ...
    obj.modal_tray:frame({
        w = tray_width,
        h = tray_height,
        -- x = cres.x + ((cres.w / 2) - (tray_width / 2)),
        x = cres.x + tray_padding_left + ((cres.w - tray_width) / 2),
        y = cres.y + tray_padding_top
    })
    print("text :: "..text)
    obj.modal_tray[1].fillColor = {hex = tray_color, alpha = tray_alpha}
    print(obj.modal_tray[1])
...
function obj:highlightScreen(cscreen)
    local color = "#ffffff"
    local highlightFillAlpha = 0.1
    local highlightFadeOutDuration = 1

    hs.canvas.new(cscreen:frame()):appendElements({
        action = "fill",
        padding = 4,
        type = "rectangle",
        fillColor = { alpha = highlightFillAlpha, hex = color },
    },
    {
        action = "stroke",
        padding = 4,
