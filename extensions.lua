...
dbgf = function (...)
  local state = debug.getinfo(2)
    -- print(hs.inspect(debug.getinfo(0)))
    -- print(hs.inspect(debug.getinfo(1)))
    -- print(hs.inspect(debug.getlocal(2)))
...
-- Formats a string with provided arguments, but allows incomplete argument lists
-- to leave format specifiers intact if no argument is provided.
-- If the replacement is a table, it converts it to a string representation.
--
-- @param format_string string: The format string containing the format specifiers (e.g., %s)
-- @param ... any: The values to be inserted into the format specifiers in the string
--
-- @returns string: The formatted string, where provided format specifiers are replaced
--                  with the corresponding arguments, and missing arguments leave
--                  the specifiers as they are.


-- Helper function to check if a format string is valid
function isValidFormatString(fmt)
    return type(fmt) == "string" and fmt:find("%%s")
end

-- Helper function to convert tables or non-string values to strings
function to_string(val)
    if type(val) == "table" then
        return hs.inspect(val)  -- Serialize table using hs.inspect
    else
        return tostring(val)
    end
end
...
function merge(...)
    local result = {}
    for _, v in ipairs({...}) do table.insert(result, v) end
    return result
end
...
function safeFormat(fmt, ...)
    ...
    if not isValidFormatString(fmt) then

        args = merge(fmt, ...)
        -- Create a format string with %s placeholders based on args count
        fmt = string.rep('%s',select('#',args),' ')
    else
        args = {...}
    end

    local arg_index = 1  -- Track which argument is being applied to the format specifier

    -- Replace format specifiers (%s) one by one
    local result = fmt:gsub("%%[s]", function(match)
        local replacement = args[arg_index] or match  -- Replace with argument or keep the format specifier
        arg_index = arg_index + 1  -- Move to the next argument
        return to_string(replacement)  -- Convert to string if necessary
    end)
...
function subRequire(path)
    ...
    print("subRequire")
    local str2 = debug.getinfo(2, "S").source:sub(2)
    print(str2)
    local str3 = debug.getinfo(2, "S").source:sub(4)
    print(str3)
